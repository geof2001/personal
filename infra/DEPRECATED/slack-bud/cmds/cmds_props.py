...
class CmdProps(CmdInterface):
    ...
    def get_help_text(self):
        return handle_props_help_command()
    ...
    def invoke_sub_command(self, sub_command, args, response_url):
        try:
            ...
            if sub_command == 'list':
                return handle_list_sub_command()
            if sub_command == 'history':
                return handle_history_sub_command()
            if sub_command == 'get':
                return handle_get_sub_command()
            if sub_command == 'set':
                return handle_set_sub_command()
            if sub_command == 'delete':
                return handle_delete_sub_command()
            if sub_command == 'restore':
                return handle_restore_sub_command()
            if sub_command == 'copy':
                return handle_copy_sub_command()
    ...
    def invoke_confirm_command(self, type, params):
        return props_confirm(params, ENVIRONMENTS)

    def is_confirm_command(self, params):
        if self.get_fallback_string_from_payload(params) == 'Properties':
            return True
        return False
...
def get_dynamo_client(environments, args):
    # ToDo make a get dynamo client just like backup.py,
    # Move to utils, and same for getting a cloudformation clinet.
    return None


def handle_list_sub_command():
    return slack_ui_util.text_command_response(
        title='Props',
        text='list sub-command'
    )


def handle_history_sub_command():
    return slack_ui_util.text_command_response(
        title='Props',
        text='history sub-command'
    )


def handle_get_sub_command():
    return slack_ui_util.text_command_response(
        title='Props',
        text='get sub-command'
    )


def handle_set_sub_command():
    return None


def handle_delete_sub_command():
    return None


def handle_restore_sub_command():
    return None


def handle_copy_sub_command():
    return None
...
def handle_props_help_command():
    """Return help response back to SlackUI"""
...
def handle_props(command, environments, args, response_url):
    ...
    try:
        ...
        props_table = bud_helper_util.get_props_table_name_for_stack_name(
            session, args.regions[0], stack_name
        )
    except:
